...
This document describes the native configuration, setup tools and acceptance checks. It is not yet a verified participant walkthrough. The dedicated workflow, page ACLs, presenter account assignments and automatic-publishing migration have been configured and checked. Real non-admin sessions have demonstrated author edits, submission, the Approver's review commands, return to Draft, approval and automatic publication of the revised summary to the live practice page. Native Content Editor additionally verified the submitted Author's read-only state and the Approver's lock, approval and history sequence. The remaining checks below must pass before the workshop instructions are published.

**Confirmed editing-surface difference:** in this tenant, the Author could still save **Summary** through Page Builder's **Content** tab after submission to **Awaiting approval**, despite the configured **Workflow State Write** denial and an empty item lock. Workflow commands respected the role restrictions. A later submitted version of the same item, opened with the same non-admin Author account in native **Content Editor**, was read-only: the interface displayed a write-access warning, **Save** and **Edit** were disabled, and **Summary** could not be changed. Use **Content Editor** for the workflow permission demonstration and **Page Builder** only for visual preview. Do not claim the two editing surfaces enforce this restriction identically. The complete revised Content Editor click path still needs its own final walkthrough before registration.
...
Both members of a pair can read their practice page. Page permissions limit editing to the assigned pair, and workflow permissions control the available commands. The submitted-version editing restriction has been verified in **Content Editor**, with the Page Builder limitation described above. The exercise uses the page's versioned **Title**, **summary** and **body** fields. Local **Data** items and the optional image remain read-only; the Resource Metadata Marketplace app is outside this exercise.
...
The proposed exercise uses **Content Editor** for authoring, commands and history, with optional **Page Builder** preview:

1. Both participants open **Content Editor**, search for `/sitecore/content/LibertyMutual/liberty-mutual-agent-portal/Home/workshop-practice/pair-NN` with their assigned pair number, and select the page under **Direct Hit**. Use **English** and the same version. The full-path search was verified for **Demo**, `pair-01`.
2. The Author selects **Review → Edit** when required to lock their **Draft**, edits **Summary**, saves and submits with a comment. Use the **Review** tab's workflow commands. After submission, refresh the same version as the Author and observe the read-only banner and disabled **Save** and **Edit** controls in **Content Editor**.
3. The Approver reviews that page in **Awaiting approval** and selects **Review → Edit** to lock the item. In the verified Approver session, **Approve** and **Return to author** were disabled until this lock was acquired. For approval, the verified sequence is **Review → Edit → Approve → Comments → OK**. The item becomes **Approved**, the review lock clears and that Approver's content is read-only. The return command sends the page back for revision; complete the native Content Editor return-path check before publishing the participant guide. Both participants can inspect comments and recorded usernames through **Review → History** in **Content Editor**. Actual Author and Approver sessions verified this history view.
4. After approval, wait for publication to complete, then open the assigned page's portal URL and compare its content with the approved version. No separate **Publish** click is part of this exercise. The live **Demo** page showed the approved revised summary after the actual Approver's approval; Page Builder progressed from **Approved** to **Live**.
5. Create another **English Draft** version for the next review cycle, leaving the previously published version unchanged. The real Author's Page Builder route was **Live Version 1 → Create version → optional name → Create**. Verify the corresponding **Content Editor** new-version route before making it part of the published participant instructions.

To inspect the page visually, open the same page and version in **Page Builder** with **Default** editing host selected. Return to **Content Editor** for workflow actions and the submitted-item read-only demonstration. A preview of draft content is not a published-result check.
...
2. Each participant accepts their invitation and opens the SitecoreAI application, such as **Page Builder** or **Content Editor**, at least once. This first sign-in establishes the native user record that the assignment tool must find.
...
## Publishing prerequisites and remaining acceptance checks
...
| Author submits | State becomes **Awaiting approval** and Author commands are unavailable. In **Content Editor**, the submitted version is read-only with **Save** and **Edit** disabled; do not assert this restriction for Page Builder's Content tab |
| Approver opens their assigned submitted page | Select **Review → Edit** to acquire the review lock; **Approve** and **Return to author** then become available, and **Submit** is not |
...
| Approver approves | **Review → Edit → Approve → Comments → OK** moves the item to **Approved**, releases the review lock and triggers publication of the approved English page to **Experience Edge** |
...
| Review history is inspected | **Content Editor → Review → History** displays the selected version's state transitions, real usernames and comments |
...
The portal's reviewer reset does not reset Sitecore content or workflow history. Repeat this exercise by creating another Draft version through the Author's normal editing flow. To restore starter wording, edit the assigned page and submit it for approval again; approval triggers publication. There is no bulk reset or deletion action in these provisioning tools.
