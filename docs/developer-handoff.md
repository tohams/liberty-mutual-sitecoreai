...
| `PERSONALIZE_MIDDLEWARE_CDP_TIMEOUT` | Positive milliseconds for the native personalization decision; portal default `2000` |
| `PORTAL_PERSONALIZATION_DIAGNOSTICS` | Opt-in safe stage diagnostics; disabled unless exactly `true`. Reports booleans, fixed outcome categories, receipt type, elapsed milliseconds and an optional numeric HTTP status only |

The portal gives native campaign discovery 1.5 seconds and each decision 2 seconds, replacing the SDK's 400 ms defaults. The supported timeout environment variables override these values; blank, invalid or nonpositive values use the portal defaults. These are bounded service-request budgets, not a fixed delay or a total page-response deadline. When native personalization cannot complete, the portal keeps neutral guidance and allows operational work to continue. Revalidate both cold and warm request latency, positive and neutral campaign outcomes, and these budgets in the customer's destination hosting region before handoff.

The decision transport enforces an aborting deadline, capped at 10 seconds even if a larger decision timeout is configured. It makes one native request per discovered campaign execution, with no decision retries. Invalid JSON, HTTP failures, unavailable identity, absent browser cookies and variants outside the native discovery allowlist produce neutral content. `PortalPersonalizeService` preserves SDK discovery and cache behavior while containing GraphQL failures before the base proxy can log an exception; opt-in diagnostics report only a fixed discovery-unavailable outcome.
...
**Handoff is blocked as of 21:28 UTC on September 10.** The required SitecoreAI deployment completed, but CM warm-up, schema population and reindex post actions failed. The CM hostname returned HTTP `530` with Cloudflare error `1000`, then authoritative `NXDOMAIN` after one standard environment restart was initiated. Page builder cannot connect to the tenant. The post-deployment personalization run recorded six failed assertions, with some steps skipped; the previous 37-assertion pass remains historical evidence. Restore managed CM DNS, Preview content and native editing, then rerun connected acceptance before closing handoff. A Sitecore support request has been drafted but not sent. See the [rotation evidence](qa-evidence-2026-09-10.md#master-context-id-retirement).
