...
Normal scoped releases push `LibertyMutual.Model`, `LibertyMutual.SitePresentation` and `LibertyMutual.SupportForm`, in that order, with `CreateAndUpdate`. Model is limited to four owned roots: templates, renderings, placeholder settings and layouts, each under its `Project/LibertyMutual` path. Its layouts include `PortalLayout`, `ResourcesLayout`, `ResourceArticleLayout`, `ProductsLayout` and `CampaignLayout`. Exact rules exclude `SupportLayout` and the global `headless-support-form` placeholder because SupportForm owns them separately. No module expands into the shared layout tree. The [component placement matrix](content-model.md#component-placement) records their contracts.

`LibertyMutual.SitePresentation` has nine exact `SingleItem` includes for the site's matching SXA placeholder restrictions. `LibertyMutual.SupportForm` adds exactly three developer-owned `SingleItem` includes: `SupportLayout`, the global `headless-support-form` placeholder and its matching site restriction. It serializes no page, native form definition or webhook. The native Form rendering remains a shared Sitecore dependency; the Support page binding uses the separate [guarded setup procedure](native-forms-operator-guide.md#configure-the-binding-in-another-environment).

With `--seed`, the release script pushes Model, creates missing Content, Taxonomy and ResourcePageBranch items, then pushes SitePresentation and SupportForm after their parent folders exist. `--seed-taxonomy` seeds only missing managed-list items on an existing site; `--seed-branch` seeds only the missing blank Resource page structure. The Campaign page branch and API-created editable campaign pages have a separate [explicit provisioning procedure](../authoring/CAMPAIGN-AUTHORING.md#provision-another-environment). Content excludes the nine SitePresentation restrictions, the exact Support form site restriction, the separately owned `Data/Taxonomy` subtree, both page branches and the API-created campaign subtrees. All seed modules preserve existing authored items. `--publish` explicitly publishes the owned site and all four model roots. It does not include related items outside that scope. Never run a broad starter-wide serialization push for this portal.

This scoped CLI procedure supports focused model updates. For authoring-environment deployments, `xmcloud.build.json` explicitly includes only `nextjs-starter`, `LibertyMutual.Model`, `LibertyMutual.SitePresentation` and `LibertyMutual.SupportForm` as Items as Resources. Initial editorial content, managed taxonomy lists and the editable Resource page branch remain outside that package and use the guarded seed procedure above. The root serialization configuration excludes unused Click Click Launch modules while retaining their files as upstream reference. Removing resource-backed Starter Kit definitions requires rebuilding and deploying the authoring environment; a frontend build or CLI item deletion cannot remove those resource files. Verify the existing portal components, native Form placement on Support and retained page layouts after an authoring deployment. The current frontend uses Git-connected Vercel deployment; a SitecoreAI Vercel Deploy App or hosting-provider connection is a separate configuration and acceptance check, not proof supplied by the authoring deployment.
...
There are seven named personas across four agency profiles and fifteen isolated reviewer packs (`01`–`15`). All 105 fictional portal accounts use the shared password `Sitecore`. Assign one pack per attendee for hosted exercises and keep the same suffix across all seven personas. For a hosted presenter journey, coordinate use of an attendee’s pack and complete the guided work and its required cleanup before that attendee begins. Never reset a pack while an attendee is using it. Roles, licenses and appointments remain the same across packs. The JSON credentials are intentionally fictional evaluation material. The provisioning script converts them into salted scrypt hashes for server-only runtime verification. Rotate them by regenerating the hash file and redeploying; never re-use them for customer employees or real insureds.
