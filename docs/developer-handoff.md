...
This scoped CLI procedure is the implemented POC model-release path. The production implementation can version the model in GitHub and release it through a SitecoreAI Deploy authoring-environment deployment after explicitly configuring the owned model modules and initial-content handling. Verify that build configuration before adopting the integrated path. The current frontend uses Git-connected Vercel deployment; a SitecoreAI Vercel Deploy App or hosting-provider connection is a separate configuration and acceptance check, not proof supplied by the authoring deployment.
