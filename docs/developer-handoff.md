...
| Redis state provider | Persistent state, atomic changes and namespace isolation | Platform administrators |
...
| `SITECORE_PROFILE_IMPORT_URL`, `SITECORE_PROFILE_IMPORT_API_KEY` | Tenant-specific native import endpoint and server-only API key for on-demand reviewer restarts; ordinary local component work does not require them |
...
| `PORTAL_VERIFIED_PROFILE_GENERATIONS` | Verified historical seed generations; new restart sets activate from their durable native import receipts rather than this static list |
...
The SitecoreAI **Demo** environment is dedicated to this Proof of Concept.
...
Node.js 24 is required. `npm ci` installs the committed dependency graph. The app's `npm run build` generates the actual Sitecore component maps and SDK metadata before compiling Next.js. Generated `.sitecore` files are ignored build artifacts, not source files to edit by hand. The component-map configuration excludes tests, fixtures and pure helpers; regeneration must retain the portal components `AgentGuidance`, `ResourceArticle`, `ResourceImage`, `ResourceSearch`, `ProductSpotlight`, `CampaignPage`, `CampaignHero`, `CampaignAlert`, `CampaignCallout`, `CampaignAccordion`, `CampaignLinkList` and `CampaignContact`, plus `PartialDesignDynamicPlaceholder` and SDK built-ins. Verify the registration set when adding modules so utility code does not become a CMS rendering.
...
* **Offline validation** runs strict lint and `npm test` across all source tests, including identity readiness, domain/authorization/state/routing tests, followed by read-only seed and UDL contract checks and native CLI validation of the seven owned serialization modules. Dependency and CLI restoration use public package networks; no tenant login is needed.
...
Normal scoped releases push `LibertyMutual.Model`, `LibertyMutual.SitePresentation`, `LibertyMutual.SupportForm`, and `LibertyMutual.ComponentLibrary`, in that order, with `CreateAndUpdate`. Model is limited to four owned roots: templates, renderings, placeholder settings and layouts, each under its `Project/LibertyMutual` path. Its layouts include `PortalLayout`, `ResourcesLayout`, `ResourceArticleLayout`, `ProductsLayout` and `CampaignLayout`. Exact rules exclude `SupportLayout` and the global `headless-support-form` placeholder because SupportForm owns them separately. No module expands into the shared layout tree. The [component placement matrix](content-model.md#component-placement) records their contracts.

`LibertyMutual.SitePresentation` has nine exact `SingleItem` includes for the site's matching SXA placeholder restrictions. `LibertyMutual.SupportForm` adds exactly three developer-owned `SingleItem` includes: `SupportLayout`, the global `headless-support-form` placeholder and its matching site restriction. It serializes no page, native form definition, or webhook. `LibertyMutual.ComponentLibrary` owns four exact `SingleItem` records for **Available Renderings** and its **Campaign**, **Resources**, and **Agent tools** sections. It maintains grouping, membership, display names, and sort order; other native library groups and editorial content remain outside its scope. The native Form rendering remains a shared Sitecore dependency; the Support page binding uses the separate [guarded setup procedure](native-forms-operator-guide.md#configure-the-binding-in-another-environment).

With `--seed`, the release script pushes Model, creates missing Content, Taxonomy and ResourcePageBranch items, then pushes SitePresentation, SupportForm, and ComponentLibrary after their parent folders exist. Finally, LibrarySeed creates any missing native library groups beneath the configured library root. `--seed-taxonomy` seeds only missing managed-list items on an existing site; `--seed-branch` seeds only the missing blank Resource page structure. The Campaign page branch and API-created editable campaign pages have a separate [explicit provisioning procedure](../authoring/CAMPAIGN-AUTHORING.md#provision-another-environment). Content excludes the nine SitePresentation restrictions, the exact Support form site restriction, the **Available Renderings** subtree, the separately owned `Data/Taxonomy` subtree, both page branches and the API-created campaign subtrees. `LibertyMutual.LibrarySeed` contains those six native groups in exact `CreateOnly / SingleItem` includes and runs only with `--seed`. All seed modules preserve existing authored items. `--publish` explicitly publishes the owned site and all four model roots. It does not include related items outside that scope. Never run a broad starter-wide serialization push for this portal.

This scoped CLI procedure supports focused model updates. For authoring-environment deployments, `xmcloud.build.json` explicitly includes only `nextjs-starter`, `LibertyMutual.Model`, `LibertyMutual.SitePresentation`, `LibertyMutual.SupportForm`, and `LibertyMutual.ComponentLibrary` as Items as Resources. Initial editorial content, managed taxonomy lists, editable page branches, and LibrarySeed remain outside that package and use the guarded seed procedure above. The root serialization configuration excludes unused Click Click Launch modules while retaining their files as upstream reference. Removing resource-backed Starter Kit definitions requires rebuilding and deploying the authoring environment; a frontend build or CLI item deletion cannot remove those resource files. Verify the existing portal components, native Form placement on Support and retained page layouts after an authoring deployment. The frontend deploys through its GitHub-to-Vercel connection. The optional SitecoreAI Vercel Deploy App provides a deployment entry point inside SitecoreAI when that integration is configured.
...
There are seven named personas across four agency profiles and twenty isolated reviewer packs (`01`–`20`). All 140 fictional portal accounts use the shared password `Sitecore`. Reserve pack `01` for workshop presenters; assign attendee and spare packs from `02`–`20` (19 non-presenter packs). Keep the same suffix across all seven personas. Never reset a pack while an attendee is using it. Roles, licenses and appointments remain the same across packs. The JSON credentials are intentionally fictional evaluation material. The provisioning script converts them into salted scrypt hashes for server-only runtime verification. Rotate them by regenerating the hash file and redeploying; never re-use them for customer employees or real insureds.
...
* `restart` imports and verifies a fresh seven-profile native set on demand, then starts fresh operational work and invalidates that pack’s older application sessions. There is no preallocated set limit. It preserves earlier CDP history and uses one durable request UUID/expected run across retries; pending or failed verification leaves the previous pack active.

Use the [auth/data runbook](../examples/liberty-mutual-agent-portal/docs/auth-and-data.md) for exact reset commands, persistent-state behavior and credential distribution. Reviewers should coordinate pack assignments: agents in one agency and pack intentionally share agency work. Saved work and pack metadata have no automatic expiry. For an existing deployment, follow [the storage upgrade procedure](../examples/liberty-mutual-agent-portal/docs/auth-and-data.md#upgrade-existing-workspace-storage-without-a-reset) to remove legacy Redis expiry while preserving record contents and versions; it performs no reset or native profile change.

Current portal capacity is twenty packs. The archived fifteen-pack seed contains 420 native profiles: 105 logins with four historical fixture generations (`0`–`3`). This is not the limit for on-demand restarts; new sets and their verification are recorded in durable operator state. Native import is verified in two receipts: `fixtures/udl/import-verification.json` retains the original 112 profiles for packs `01`–`04`; `fixtures/udl/import-verification-packs-05-15.json` records the additional 308 profiles with zero errors in batch `e538f430-54dc-4606-b4f2-82a3e53104ec`. Both exact payloads are preserved in `fixtures/udl/imports`. Those receipts cover packs `01`–`15`; packs `16`–`20` require their own verified on-demand profile initialization. Existing profiles were not reimported or reset. Import completion does not itself prove runtime identity linking or native decisions. The import's correlation UUID differs from its opaque business identifier. Keep the `liberty-mutual-agent` provider and runtime identity mapping aligned. Use the auth/data runbook's explicit `--packs` export into a separate staging directory only when deliberately extending the archived seed; ordinary restarts use the protected operator workflow. Do not re-run exports merely to refresh timestamps: new correlation IDs change the verified artifact checksum. Each on-demand restart requires a verified seven-profile result before activation. To inspect a current profile, use the signed-in bootstrap’s `udlIdentity.id`; the static identity map remains an archive of the initial seed.
