...
**Start:** Design and Agentic access, the existing ABM conversation, and optional Avery/Jordan/Maya portal accounts. The standard loop inspects existing native output; a new agent run is not required for a repeatable presentation.

1. Open **Design → Brand kits → Liberty Mutual — Independent Agents**. In **Knowledge**, inspect the published PDF (13 pages and 13 knowledge chunks). Open **Sections → Brand Context**, then **Back to sections → Visual Guidelines → Visual guidance**; inspect the authored palette, Roboto and **Non AI editable** setting. In **Channels → Liberty Mutual Agent Portal → Localization → Configuration**, confirm the English Brand kit assignment. Explain its public official sources and original portal direction; it is not a customer-approved corporate manual.
2. In **Conversations**, open **Liberty Mutual — Expand your small-business practice**. The [existing entry link](https://app.sitecorecloud.io/agentic/chat-new/944bcaa4-a10d-4bec-93bc-ebcff9c86328?organization=org_XqL3u1MSNVuubOTb&tenantId=97eea84c-ac47-4d91-7e4f-08defdaaa7df) redirects to Conversations.
3. Inspect **Classic** and its eight rows marked **Done**, across **Account Data Enricher**, **Brief Generator** and **Bulk Content Generator**. This completed conversation currently exposes no **Run workflow** control. Keep its run history unchanged. Production figures and the growth cohort are supplied fictional account inputs; no live CRM enrichment has been demonstrated.
...
5. In **Page Builder → Home → My workspace → Layers → AgentGuidance → Edit personalization rules**, inspect the Live **Liberty Mutual - Small business growth guidance** table with three role rows and original neutral content. Click **Cancel**. Trace the package to the existing authored `AgentGuidance` datasources and native role/cohort decision. Use loop 1 to show the portal's distinct next steps. Explain that the configured portal campaign is inspectable; the generation run did not automatically publish those datasources.
...
**Inspect current grounding:** open **Liberty Mutual — Brand-grounded ABM creative brief**, the [saved September 14 conversation](https://agentic-studio-use.sitecorecloud.io/conversations/6605d4e6-b423-4510-afd0-161f2ee99dc8), expand **Get Brand Kit Section** for **Visual Guidelines**, then open **Artifacts → Liberty Mutual Independent Agents | Small-Business Resources ABM Creative Brief**. Its palette, typography and logo direction come from the expanded native Visual guidance; the request supplied no expected colors. This separate saved creative brief verifies consumption of authored text; it still requires human review before publication or activation. Conflicting top-level colors and empty-array projection persist, and the original ABM still records an excerpt input. Follow the [repair checklist](agentic-studio/brand-kit-consumption-repair-2026-09-14.md) for a deliberate fresh check; do not rerun the completed campaign just to inspect saved proof.
