...
**Evidence baseline:** the state-eligibility release in [PR #14](https://github.com/tohams/liberty-mutual-sitecoreai/pull/14), merged September 11, 2026 UTC. That release passed 80 application tests, connected builds, owned serialization validation, deployed eligibility checks and 37 native personalization checks in both preview and production. See [release evidence](qa-evidence-2026-09-11.md) and the PR's production results. This runbook assembles those verified paths with the existing editorial and Agentic walkthroughs; it does not claim a new rehearsal of every loop or completion of the outstanding customer-role and Brand Kit retrieval checks.
...
4. Use the synthetic password **`Sitecore`** for every portal login. The examples below use pack `01`; substitute the assigned suffix consistently. Switch people through **Your profile → Sign out**, then sign in again. The recorded cross-agent cases referenced in loop 7 belong specifically to **preview pack `04`**.
...
**Start:** native Page Builder access, the correct customer author and approver roles, and an agreed temporary edit plus restoration wording. **Customer-role approval acceptance remains pending.** Until rehearsed with those roles, present inspection and preview as the verified portion. Plan for Search indexing time.
...
3. Make the agreed small summary edit without changing regulatory meaning. Preview it through the editing host.
4. With the appropriate roles, complete the installed workflow's actual approval step and publish the exact owned item. Observe the native workflow states rather than assuming a seed item's Live status proves approval behavior.
5. Verify the published article and resource listing. After native Search indexes the change, verify its result card. No Git deployment is required.
6. On **workspace**, inspect **AgentGuidance**, the reusable **Data/Guidance** datasources, and the **Default** and **Highlight** variants. Show the eyebrow, headline, body and action-link fields that marketing owns.
...
**Explain:** “Content changes and application releases have separate ownership and release paths.” The editing host isolates operational preview data, but publishing in this shared Sitecore tenant can update the live site. A reviewer-pack reset does not undo a CMS publication.

**If the transition is not ready:** show the saved version, preview and last verified native evidence. Do not bypass approval or claim immediate indexing. Do not perform a broad site publication to force one card to change.

**Leave it ready:** restore agreed rehearsal wording through the same workflow, or explicitly retain the approved change. Record the item/version and publication result. See [content model and publishing](content-model.md) and [customer-role acceptance](developer-handoff.md#runtime-acceptance-and-evidence).
...
**Start:** GitHub/code access and a coordinated preview pack. Avery and Jordan must share the same pack. The recorded cross-agent records in [September 11 evidence](qa-evidence-2026-09-11.md) are in preview pack `04`; they are not production records or pack `01` records. Check that the run is still active before relying on their IDs.

1. Briefly show Daniel's licensed-only Products selector and retained state through intake, or reuse the result of loop 3.
2. In preview, have Avery create a new Florida BOP draft and note its ID/reference. Sign out and sign in as Jordan from the same pack. Open that shared record: Florida remains visible, with an authority explanation and disabled Edit/checklist/Submit controls. The retained recorded Avery case is already Submitted, so it shows restricted read access but is not a Draft-button demonstration; create a fresh draft if that comparison is needed.
