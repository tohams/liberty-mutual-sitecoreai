...
**Loop 8 status:** **Liberty Mutual Small Business Guide CTA** is Live. Its exact guide goal was saved and reopened before activation on September 14, 2026 UTC. Published A/B content passed 18 checks and a normal production Daniel `.04` visit reached the guide. Native reporting at approximately **23:11 UTC on September 15, 2026** showed credited goals for both variants, with **Test is in progress**. The preceding **Liberty Mutual Small Business Resource CTA** was ended with control retained; its historical report is separate. See the [current experiment and dated reporting record](ab-testing.md#native-configuration-and-acceptance-record) and [September 14 configuration record](qa-ab-testing-2026-09-14.md).
...
**Start:** native Page Builder/A/B and results access, the [current experiment's acceptance record](ab-testing.md#native-configuration-and-acceptance-record), and an assigned portal account. **Liberty Mutual Small Business Guide CTA** is Live. Published treatments and normal guide navigation are verified. The September 15 native aggregate report credited goals to both variants and remained in progress; its small sample establishes no winner. Use the existing test rather than creating another. A normal authenticated visit adds native history; do not promise a particular treatment or stable assignment. The ended **Liberty Mutual Small Business Resource CTA** report describes an earlier test.
...
**If the latest visit is not yet reflected:** show the dated report and verified treatments, identify the unconfirmed visit-level evidence and preserve the actual reporting status. Do not force identities or generate repeat visits to make a winner appear.
...
**Start:** coordinated Daniel and Maya accounts on the [production portal](https://liberty-mutual-agent-portal.vercel.app/login), access to native Affinities, Profiles and **Liberty Mutual - Product interest spotlight**, and the [affinity acceptance record](affinity-personalization.md#acceptance-record). The historical accepted topic journeys used pack `03` on preview and pack `04` on production, generation 0; Elena `.04` provided the no-affinity comparison on both hosts. These are evidence references, not instructions to switch packs. Keep the assigned pack on production throughout this replay. Before any tagged browsing, inspect the current generation and starting scores for Daniel, Maya and Elena. For a fresh transition, run this loop before other tagged-page visits, or coordinate a verified on-demand restart as described under [Before presenting](#before-presenting). A fresh login is not a clean affinity baseline.
...
| `restart` reset | Imports and verifies a fresh seven-profile set, then starts a fresh operational run and invalidates the pack’s old sessions | Repeat a native-personalization journey with new identities; no fixed pool of sets |

Neither reset erases prior Sitecore analytics or restores CMS content. Other packs remain separate. Restart creates new identities rather than clearing existing profiles, with no preallocated set limit. Verify the new active identifiers and starting affinity scores before tagged browsing. If import is pending, failed or uncertain, the previous pack remains active; resume the same operation as the runbook directs, without starting another upload automatically. Announce a planned pack reset to active reviewers and have them refresh or sign in again as the operator runbook requires. Do not reset merely to tidy up four newly created presentation records while another person is reviewing them.
