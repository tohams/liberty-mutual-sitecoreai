...
**Loop 9 status:** the corrected preview has passed both topic journeys: Daniel built native workers-compensation affinities and Maya built household affinities, each reaching 2 views and score 1.00 before the matching authored Products spotlight rendered. The correction sends CMS page names, matching the native affinity assignments. Elena’s no-affinity profile retained neutral content after signing out the other agents, and all 37 existing Workspace checks passed on preview. Preview acceptance is complete; production verification follows deployment. See the [affinity contract](affinity-personalization.md) and [September 13 QA record](qa-affinity-personalization-2026-09-13.md).
...
Loop 9 can accompany loop 1 to distinguish known agent attributes from browsing-derived interest. Use the accepted preview until production verification is recorded. Loop 1 uses role and an imported growth cohort; loop 8 uses experiment allocation; loop 9 uses affinity conditions on a separate Products component. Keep their evidence and outcomes separate.
...
**Start:** a coordinated Daniel preview account, access to native Affinities, Profiles and **Liberty Mutual - Product interest spotlight**, and the [affinity acceptance record](affinity-personalization.md#acceptance-record). The accepted rehearsal used `daniel.03` and `maya.03`, generation 0, with `elena.04` as the no-affinity comparison. Native setup and all three corrected preview journeys are verified. Inspect the starting profile's scores before rehearsal. A fresh login is not a clean affinity baseline.
...
**Current acceptance status:** corrected preview `cd96c0d` verified `daniel.03` and `maya.03`, both generation 0. Their separate journeys each built **2 native views, score 1.00**, for workers compensation and household respectively. Each matching authored spotlight appeared, and its CTA retained the selected risk state. The original failed journeys remain in the [QA record](qa-affinity-personalization-2026-09-13.md). Elena’s independently checked no-affinity profile retained neutral content, and the corrected preview passed 37 existing Workspace personalization checks. Production verification remains separate; use the accepted preview until that result is recorded.
