...
**Loop 9 status:** the Products page, three approved datasources and **Liberty Mutual - Product interest spotlight** table are live, with the dedicated layout and five saved affinity assignments. Two actual topic journeys remained neutral and their inspected profiles had no populated affinities. Present this as a configured capability awaiting runtime acceptance; the [affinity contract](affinity-personalization.md) and [September 13 QA record](qa-affinity-personalization-2026-09-13.md) separate the completed setup from the missing score-to-content transition. The earlier seven-loop rehearsal does not establish acceptance of this added loop.
...
| [9](#loop-9-an-interest-becomes-a-relevant-next-step) | An interest becomes a relevant next step | Marketing, personalization and platform owners | 5–7 minutes after runtime acceptance |
...
Loop 9 can accompany loop 1 to distinguish known agent attributes from browsing-derived interest, once its native acceptance is complete. Loop 1 uses role and an imported growth cohort; loop 8 uses experiment allocation; loop 9 uses affinity conditions on a separate Products component. Keep their evidence and outcomes separate.
...
3. Close the account, return to **My workspace → Your priorities → All priorities**, and reload. Show the new priority attached to the policy. Saved follow-ups are not in the workspace's **Renewals** filter. Explain the separation between saving an administrative task and changing insurance coverage.
...
1. In the portal site, locate **Home → resources → texas-workers-compensation**. Select **ResourceArticle** and inspect its title, summary, body, source and reviewed date. The page itself is the datasource; the unused **Data/Resources** copies are not the active source.
2. Show the current article and Search result in another tab. Record the original summary.
3. Open that same resource item in native **Content** and create a **new English draft version** with a descriptive name, such as **Presenter verification**. Edit its **summary** field and save the agreed small change without altering regulatory meaning, then preview this version through the editing host. Record the new version number; do not edit the published version in place.
...
5. Choose **Publish Page** for the current page in **English**, with **Subpages** off and **All references** off. Confirm the scoped publication completes, then verify the new summary in the live article and resource listing.
...
7. On **workspace**, inspect **AgentGuidance**, the reusable **Data/Guidance** datasources, and the **Default** and **Highlight** variants. Show the eyebrow, headline, body and action-link fields that marketing owns.
...
3. Inspect the Cedar Ridge account inputs and the **Account Data Enricher**, **Brief Generator** and **Bulk Content Generator** stages and run history. Production figures and the growth cohort are supplied fictional account inputs; no live CRM enrichment has been demonstrated.
4. Open the eighth artifact, **Cedar Ridge Insurance ABM — Final Reviewed Implementation Package**, in Content, JSON or Preview. Review the principal, producer, account-manager and neutral payloads and the corrected audience contract. Its title does not constitute human approval.
...
**Expected result:** a published Brand Kit, native agent execution history and native generated artifacts are available. The eighth artifact carries the corrected audience rules and four validated payloads. No email delivery, automatic publishing or measured conversion lift is implied.

**Required qualification:** the Brand Kit was selected as context, but successful native retrieval of its contents remains **unverified**. Revised generation used a supplied, source-grounded brand excerpt. Show that boundary directly; a selected attachment is not evidence that an agent consumed it. See the [capability ledger](agentic-studio/capability-ledger.md).
...
**Start:** coordinated preview account **`daniel.04`**, access to native Affinities, Profiles and **Liberty Mutual - Product interest spotlight**, and the [affinity acceptance record](affinity-personalization.md#acceptance-record). The native setup is live; use the sequence below for acceptance once the missing score population is resolved. Inspect the starting profile's scores before rehearsal. A fresh login is not a clean affinity baseline.
...
3. As Daniel, visit the Illinois and Texas workers-compensation pages. Observe the native page events and resulting interest score on the same identified profile. No exact visit count or guaranteed update delay has been verified.
4. Return to **Products & appetite** with Illinois still selected. The table uses one built-in **Custom Value → Top Affinity** String input (`top_affinity_value`), with exact matches for `workers_compensation` and `household`. When it returns `workers_compensation`, verify **Build a stronger workers compensation conversation** and **Review account preparation**. Record the score, input output, native decision and rendered datasource; Daniel's known attributes stay unchanged.
...
6. Inspect an identified comparison agent whose scores do not match a topic condition and verify neutral content. A household comparison requires a separately checked starting profile so earlier workers-compensation activity does not confound the result.

**Expected result after acceptance:** actual tagged-page visits build a native affinity signal that selects an authored ProductSpotlight variant for the same agent. A saved tag, previewed variant or configured condition alone does not prove that transition. Keep the native decision/fallback, same-profile journey and comparison evidence in the [affinity record](affinity-personalization.md).

**Input boundary:** the built-in custom value has no affinity-name selector in the inspected UI. The site currently uses only `insurance_interest`; do not describe the input as explicitly scoped to it. No valid affinities returns `null` according to Sitecore's documentation; neutral delivery still needs runtime verification. The inspected built-in source retains the first encountered value when scores tie. That is not a business priority or a guarantee of stable topic ordering; see the [source-contract evidence](affinity-personalization.md#saved-native-decision-and-built-in-contract).
...
**Current observed result:** the workers-compensation journey and a separate household journey both retained the neutral headline, with no populated affinities on the inspected profiles. Show the live configuration and explain the unresolved native scoring check using the [QA record](qa-affinity-personalization-2026-09-13.md). Do not alter known attributes or force a variant to produce the expected screen.
...
- Affinity profile generation, starting/ending topic scores, native condition/decision and rendered spotlight, comparison result; any pending checks:
