...
5. Check the assigned pack's starting state. Saved agency work lasts seven days after its latest saved change; viewing it does not extend that retention. If it has expired, the next visit restores the agency's starting fixtures without changing native identities or CDP history. Pack age does not block sign-in; application sessions last eight hours. Check current product-rule dates before a transactional exercise; the business-metrics scenario is fixed while authorization uses current UTC.
