...
- Site authoring restrictions: ten exact SitePresentation items plus the separate Support form restriction under `/sitecore/content/LibertyMutual/liberty-mutual-agent-portal/Presentation/Placeholder Settings`

`LibertyMutual.Model` permits **CreateAndUpdate** only for the four model roots, with exact exclusions for `SupportLayout` and the global `headless-support-form` placeholder. `LibertyMutual.SitePresentation` permits **CreateAndUpdate** for ten exact **SingleItem** includes, one per named site placeholder restriction. The separate `LibertyMutual.SupportForm` module owns exactly three **CreateAndUpdate / SingleItem** items: `SupportLayout`, the global `headless-support-form` placeholder and its matching site restriction. It contains no page, native form definition, or webhook. `LibertyMutual.ComponentLibrary` owns four exact **CreateAndUpdate / SingleItem** records: the **Available Renderings** root and its **Campaign**, **Resources**, and **Agent tools** sections. It manages grouping, membership, display names, and sort order without owning the other native library groups or editorial content.

`LibertyMutual.Content` permits **CreateOnly** for initial site scaffolding and editorial content, with exact **Ignored** rules excluding those ten SitePresentation restrictions, the separately owned Support form site restriction, the **Available Renderings** subtree, the editable `Data/Taxonomy` and `Data/ProductCatalog` subtrees, all three page branches, and the API-created campaign, practice, and scheduling subtrees. `LibertyMutual.Taxonomy`, `LibertyMutual.ProductCatalog`, `LibertyMutual.ResourcePageBranch`, `LibertyMutual.ProductPageBranch`, and `LibertyMutual.CampaignPageBranch` each own their bounded subtree with **CreateOnly** and stay outside authoring resource packages. `LibertyMutual.LibrarySeed` supplies six native library groups through exact **CreateOnly / SingleItem** includes. An explicit `--seed` release installs them after ComponentLibrary has created their parent; normal releases preserve their current configuration. No module permits deletion. The scoped presentation modules include no pages, datasources or broader Presentation subtree. Normal releases update Model, SitePresentation, SupportForm, and ComponentLibrary in that order; existing marketer content is never overwritten by the seed deployment script. No upstream starter module is pushed.

The root `sitecore.json` discovers only the default rendering-host module and the eleven Liberty Mutual modules. The upstream Click Click Launch files remain in Git as reference, outside this portal's serialization configuration. None of its 55 rendering definitions is used by the portal. The native dependency audit also found no references from Liberty Mutual content or model items to the Starter Kit's templates, branches, settings, placeholders, scripts or media.

`xmcloud.build.json` explicitly packages only `nextjs-starter`, `LibertyMutual.Model`, `LibertyMutual.SitePresentation`, `LibertyMutual.SupportForm`, and `LibertyMutual.ComponentLibrary` for an authoring-environment deployment. `LibertyMutual.Content`, `LibertyMutual.Taxonomy`, `LibertyMutual.ResourcePageBranch`, `LibertyMutual.CampaignPageBranch`, `LibertyMutual.ProductPageBranch`, `LibertyMutual.ProductCatalog`, and `LibertyMutual.LibrarySeed` stay outside Items as Resources (IAR); create their missing items deliberately using the guarded CreateOnly procedures. Resource-backed Starter Kit components cannot be deleted in Content Editor. Rebuild and deploy the authoring environment with this configuration to remove the old resources, then verify the component library and retained page layouts. Keep Sitecore's shared Foundation/Feature components intact. See [Sitecore's build configuration](https://doc.sitecore.com/sai/en/developers/sitecoreai/deploying-sitecoreai/the-sitecoreai-build-configuration/the-sitecoreai-build-configuration.html).
...
## Author-facing component library

Page Builder groups components by authoring purpose. Use the short display name in **Components** and **Layers**; the registered names below identify their React implementations and serialized contracts. Each component has a purpose-specific icon. Existing component IDs, datasource bindings, personalization rules, and placeholder restrictions are preserved.

| Library section | Display name | Registered component |
|---|---|---|
| Campaign | Layout | `CampaignPage` |
| Campaign | Heading | `CampaignHero` |
| Campaign | Alert | `CampaignAlert` |
| Campaign | Callout | `CampaignCallout` |
| Campaign | Accordion | `CampaignAccordion` |
| Campaign | Links | `CampaignLinkList` |
| Campaign | Contact | `CampaignContact` |
| Resources | Article | `ResourceArticle` |
| Resources | Image | `ResourceImage` |
| Resources | Search | `ResourceSearch` |
| Agent tools | Guidance | `AgentGuidance` |
| Agent tools | Spotlight | `ProductSpotlight` |
| Agent tools | Product details | `ProductDetails` |

Native experiments can retain the component name captured when they were created. The existing Resources test still identifies its control as **AgentGuidance (control)**; select **Guidance** in **Layers** to open it.
...
| `ProductDetails` | `Title`: inherited Single-Line Text; `catalogSummary`: Multi-Line Text; `catalogImage`: Image; `catalogBody`: Rich Text; `catalogProducts`: Multilist; `catalogChannel`: Droplist | `Default` |
...
`PortalRenderingParameters` inherits the installed SXA headless parameter bases, including rendering variants. Each owned placeholder has a global definition under `/sitecore/layout/Placeholder Settings/Project/LibertyMutual` and a matching SXA restriction under the site's `Presentation/Placeholder Settings`. Both use the same key and an explicit nonempty **Allowed Controls** list. The original portal slots each allow one rendering; the campaign main and sidebar regions permit their small, purpose-specific component groups. Site restriction items use the installed **Foundation/JSS Experience Accelerator/Placeholder Settings/Placeholder** template, which inherits the standard placeholder contract. The site's Available Renderings groups and Headless Variants expose the component exports. Adding a new rendering requires the component code, model item, datasource contract and allowed-controls update together.

The authored campaign adds `CampaignPage`, `CampaignHero`, `CampaignAlert`, `CampaignCallout`, `CampaignAccordion`, `CampaignLinkList` and `CampaignContact`. Their fields, page-local data, workflow and empty branch are documented in the [campaign authoring guide](../authoring/CAMPAIGN-AUTHORING.md). That guide also distinguishes the rich-text alert's rendering window from native page publication and expiration.

### Component placement

| Page | Owned layout | Placeholder → allowed rendering |
|---|---|---|
| Home (My workspace), quote, clients, agency growth, and existing product guidance pages | `PortalLayout` | `headless-agent-guidance` → `AgentGuidance` |
| Support (`/support`) | `SupportLayout` | `headless-agent-guidance` → `AgentGuidance`; `headless-support-form` → native `Form` |
...
| New product pages (`/products/<slug>`) | `ProductPageLayout` | `headless-product-details` → `ProductDetails` |
...
| Campaign pages (`/growth/<slug>`) | `CampaignLayout` | `headless-campaign-page` → `CampaignPage`; nested `headless-campaign-hero-{*}` → `CampaignHero`; `headless-campaign-main-{*}` → `CampaignAlert`, `CampaignCallout`, `CampaignAccordion`; `headless-campaign-sidebar-{*}` → `CampaignLinkList`, `CampaignContact` |
...
The rendering definitions also record versioned **Allowed on templates** values: `AgentGuidance` permits the tenant's `Page` template used by Home and `PortalPage`; `ResourceSearch` and `ProductSpotlight` permit `PortalPage`; `ResourceArticle` and its nested `ResourceImage` permit `ResourcePage`; `ProductDetails` permits `ProductPage`. These template allowlists supplement the explicit placeholder contract. They do not replace it: the two landing pages share `PortalPage` with other routes, and `ResourcePage` derives from `PortalPage`. The distinct layout and placeholder bindings keep Search, Spotlight and article content in their intended locations without relying on template inheritance to exclude pages.

The route-level placeholders, including `headless-product-details`, are static root placeholders. The image uses the native SXA dynamic placeholder inside ResourceArticle: settings key `headless-resource-image-{*}`, parent `DynamicPlaceholderId=1`, and branch placement `/headless-resource-article/headless-resource-image-1`. CampaignPage uses the same native dynamic-placeholder pattern for its three nested regions. Both parent renderings enable `IsRenderingsWithDynamicPlaceholders=true`; their rendering parameters inherit `IDynamicPlaceholder`. Shared Sitecore Foundation placeholders and the shared Headless Layout remain unchanged. The old project-owned `headless-main` definition is retired from source; no current portal layout or rendering placement should reference it. Normal CreateAndUpdate model releases do not delete obsolete items. Existing environments require the explicit presentation migration described below, followed by verification in Page builder and Experience Edge.
...
`PortalPage` inherits the tenant-generated `Page`, preserving the installed SXA headless behaviors. Its standard values select the owned `PortalLayout` and a native default workflow. `ResourcePage` overrides the inherited layout with `ResourceArticleLayout`; `CampaignPage` selects `CampaignLayout`; new `ProductPage` items use `ProductPageLayout`. Existing product guidance pages preserve their explicitly assigned guidance layouts. Home uses the tenant's project-owned `Page` template directly and explicitly selects `PortalLayout`; that Page template's standard values also select `PortalLayout` for newly created direct Page items. Shared Sitecore templates remain unchanged. The Resources and Products landing pages remain `PortalPage` items with explicit `ResourcesLayout` and `ProductsLayout` assignments. Support retains its existing page identity and uses `SupportLayout`; its native Form selection is installed separately through the [guarded binding procedure](native-forms-operator-guide.md#configure-the-binding-in-another-environment), preserving its existing guidance. Native form definitions, webhook settings and that page binding are not part of the SupportForm serialization module.
...
The existing Home item is the sole My workspace page at `/`. It keeps its native item identity and owns the workspace guidance composition. The former `/workspace` child is removed, with no redirect or alias. Other top-level routes are `/quote`, `/clients`, `/products`, `/growth`, `/resources`, and `/support`. The seven existing product guidance pages sit below `/products`; authors can add further catalog pages there using the **Product page** branch. Twelve resource articles sit below `/resources/<slug>`.
...
The catalog reads published `ProductPage` items directly beneath `Home/products`, displayed as **Home → Products & appetite**. Each page owns its inherited `Title`, `catalogSummary`, optional `catalogImage`, `catalogBody`, `catalogProducts`, and `catalogChannel`. The **Product page** branch creates one Draft page with its own empty Data folder and a `ProductDetails` rendering whose datasource is that page. The branch insertion rule is limited to the Products parent. Authors can create, approve, and publish another complete page, then refresh the portal to see its card without an application deployment or Search reindex. See [Product catalog authoring](product-catalog-authoring.md).

`catalogProducts` selects `ProductReference` items under `Data/ProductCatalog/Products`. The 18 initial references hold stable `productId` values from the operational adapter and readable display names. A reference selects which existing products a page explains; it does not define availability or grant transaction access.

`catalogChannel` is a Droplist sourced from `/sitecore/content/LibertyMutual/liberty-mutual-agent-portal/Data/ProductCatalog/Channels`. Its managed Sitecore choices are named `all`, `independent`, and `wholesale`; the selected item name remains the stored field value, and new pages still default to `all`. The portal combines that audience with the agent’s active licensed state, the chosen business-line filter, and current product availability. **Prepare account** still requires full operational eligibility.

Only complete catalog records appear: a published title, summary, recognized channel, and valid product references are required. An image is optional; a card without one uses its business-line icon. Other page types and folders are not catalog entries. The seven existing guidance pages gain catalog metadata while retaining their current authored guidance, links, and composition; unrelated authored pages are not automatically converted. New template fields, layout, rendering, and placeholder definitions belong to the model release. The editable branch uses **CreateOnly**, and the product references and managed channel choices share the existing **CreateOnly** ProductCatalog module.

## Workflow and publication

Use Page builder's **Default** editing host for draft inspection. Verify published changes, expiration and native personalization on the [production portal](https://liberty-mutual-agent-portal.vercel.app/login), which uses Live content. The editing preview has Preview content access and can render unpublished or expired CMS items; its visible content is not evidence that a Live publication succeeded.
...
The script verifies the bounded CreateOnly scopes, their exclusions, and their omission from Items as Resources before allowing any push. A normal release pushes Model, SitePresentation, SupportForm, and ComponentLibrary. With `--seed`, it first pushes Model and creates missing Content, Taxonomy, ResourcePageBranch, ProductCatalog, and ProductPageBranch items; it then applies the site presentation modules and creates missing LibrarySeed groups. On an existing site, `--seed-taxonomy` seeds only missing managed-list items, `--seed-branch` seeds only the missing blank Resource page structure, and `--seed-products` seeds only missing product references, managed channel choices, and the blank Product page branch. Existing insert options and branch rules are preserved when the separate product-catalog configuration step adds the Product page option. CampaignPageBranch and the editable campaign pages use the separate [campaign provisioning procedure](../authoring/CAMPAIGN-AUTHORING.md#provision-another-environment). Every seed module preserves existing authored items. The generator `authoring/scripts/build-content-seed.py` writes deterministic UUIDv5 items from `docs/brand/portal-content-seeds.json` and records all generated files. Run it deliberately for seed development, not on every application build; it can rewrite the local seed snapshot. It never pushes or publishes. It can replace locally captured page versions, including Resources v2; preserve and compare the current native exports before deliberately regenerating the initial seed. The separate `build-product-spotlight-seed.py` generator adds only the spotlight model and seeds. Use the guarded placement procedure in the [developer handoff](developer-handoff.md#productspotlight-release-path) for an existing site, rather than regenerating its authored pages.

The additive product generator is `authoring/scripts/build-product-catalog-seed.py`. Its `--check` mode verifies the 45 new items and manifest without writing. `build-product-catalog-seed.test.py` verifies preservation of edited branches and reference labels, and rejection of conflicting model identities. CI runs these checks, the product configuration preservation tests, and serialization validation for the two CreateOnly product modules.
...
The authored `state` values are `TX`, `FL`, `IL` or `All` (nationwide guidance). The signed-in Search default combines the agent's active licensed states with `All` in one native OR facet filter. **Risk state** offers **My licensed states**, each active licensed state, and **Nationwide guidance only**; there is no broader **All states** override. The server applies the same licensed-state scope to direct article URLs, resource bootstrap and saved-resource views. Verified native Sitecore authoring remains separate and can edit all articles. See the [developer handoff](developer-handoff.md#licensed-state-content-relevance).
