...
- Hands-on component editing and AI assistance use each participant’s unpublished **Home → Practice → Practice NN** page and its own **Data** items. Participants edit a field, check native component choices, add/reorder/remove a temporary component, and restore the starting content. Shared resource editing and publication, resource-page creation, and the Author/Approver workflow remain presenter-led. These Practice pages do not change the separate presenter workflow or require attendee author/approver pairs.
...
- Guide instructions adapt shared portal account suffixes to the signed-in participant's **workshop number**. Each number identifies seven agent logins with the same suffix and one Practice page with its local datasources; **01** is reserved for presenters. The number does not isolate shared media, component definitions, or site settings, and does not grant SitecoreAI permissions. Local exercises retain their isolated `.01` accounts.
...
- `/workshops/reset` provides self-service workspace controls. Choose a **Workshop number** from `01`–`20`; the page initially selects the signed-in participant's number and shows the seven current profile identities associated with that number.
...
- CMS Practice pages stay unpublished. Participants restore edited text and remove their temporary component placements explicitly; the portal reset does not reset a CMS page or its Data items. See [Practice page authoring](../authoring-practice.md).
