...
The native ABM space **Liberty Mutual — Expand your small-business practice** has been created and executed in the customer's tenant. Three initial runs produced six artifacts; two subsequent Content Generator runs produced the reviewed package and its audience-contract correction. Use the eighth artifact, **Cedar Ridge Insurance ABM — Final Reviewed Implementation Package**, for the walkthrough. Its four seed payloads and strict audience rules were statically checked. The September 10 package records use of the reviewed brand excerpt when native retrieval was unavailable. A separate September 13 read-only conversation verified kit and section retrieval, with an unresolved projected-color mismatch. See the [retrieval record](brand-kit-retrieval-2026-09-13.md).
...
| Subsequent content generation | Two additional artifacts; eighth artifact contains the corrected audience contract and four validated seed payloads |
...
| Native kit retrieval | Verified separately on September 13 for the kit and all nine sections. The September 10 ABM used an excerpt; projected colors remain inconsistent |

The installed ABM template uses Account Data Enricher, Brief Generator and Bulk Content Generator. The published Liberty Mutual kit was selected as context for all three stages. The run and artifact counts above record native execution, not content approval or successful retrieval. Initial run IDs and later artifact timestamps are recorded in the capability ledger. The eighth artifact's complete body was verified in native JSON, Content and Preview views.
...
3. Review the eighth artifact and its excerpt provenance. For current retrieval proof, open the existing **Liberty Mutual — Read-only Brand Kit retrieval verification** conversation and inspect its Get Brand Kit and Get Brand Kit Section results. Keep the completed workflow and all eight artifacts unchanged during a walkthrough.
4. For a deliberate new content run or destination tenant, record the actual run ID, time, returned tool evidence, artifact IDs and human review outcome. A selected kit name or an agent's assertion is insufficient evidence of retrieval. Review the authored Visual guidance and any projected-color discrepancy before using generated copy.
...
The marketing walkthrough translates one agency-level opportunity into useful role-specific content, with brand and source review before approval. Present the final package as an internal draft. Distinguish its historical excerpt input from the separate verified retrieval and remaining color-metadata limitation. IT can inspect the native space, fictional inputs, stable identifiers, generated artifacts and separate editorial publishing boundary. UDL audience matching and server-side authorization remain outside the drafting workflow; a content-personalization decision never grants access to agency records.
