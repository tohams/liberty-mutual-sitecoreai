...
The dedicated [ProductsLayout](../authoring/items/liberty-mutual/items/layouts/LibertyMutual/ProductsLayout.yml) (`75cfc4f6-9a69-52b3-97c3-11a6f1fabe89`) declares the existing standard roots plus **`headless-products-spotlight`**. Its [placeholder settings](../authoring/items/liberty-mutual/items/placeholders/LibertyMutual/headless-products-spotlight.yml) allow `ProductSpotlight`. Products English version 2 now has this dedicated layout and spotlight placement, and the owned Available Renderings list includes the component. The shared site layout remains unchanged. The [manifest](../authoring/items/liberty-mutual/content-manifest.json) records the page, layout, rendering, placeholder and datasource identifiers; live delivery must be verified separately from model definitions and saved native placement.

On `/products`, the server creates the spotlight `AppPlaceholder` and passes it through `PortalApp.productsSpotlight` into `ProductsScreen.spotlight`. Existing `headless-main` content retains its separate position. Product detail pages continue to use their existing content composition. Until a published spotlight placement exists, the original static hero remains. An empty datasource also retains that fallback in normal rendering; editing mode shows a datasource-selection prompt.
...
The preview and production portal have separate operational namespaces but share the Sitecore tenant and native profile identity for the same pack, person and generation. Preview browsing can therefore affect later native behavior for that same identity on production.
...
A `restart` affects the **entire reviewer pack**, invalidates its existing application sessions, and advances to the next preimported, verified profile generation. Earlier profiles and their history remain. Before using restart for a clean rehearsal, coordinate the pack and check that the next verified generation is available and unused for the intended native baseline; “preimported” does not mean “unvisited.” Restart stops if no next verified generation exists. Follow the [operator reset runbook](../examples/liberty-mutual-agent-portal/docs/auth-and-data.md#durable-work-and-resetting); do not reimport the same person as a substitute for deleting native history.
...
| Node 24 tests (93), lint, typecheck, production build; GitHub CI | Passed for `de30ff301acdb32621138d75db6d4db672a6e428` |
| Vercel preview and stable designated preview alias | Ready at the same commit |
| Existing Workspace native personalization regression | 37/37 production and 37/37 preview under Node 24.19.0; no resets or decision retries; separate from affinity acceptance |
