...
## Licensed-state resource defaults

[Release PR 13](https://github.com/tohams/liberty-mutual-sitecoreai/pull/13) adds a profile-aware default to native Search and workspace recommendations. The application implementation at `be4e59fd3c072ac6b4a46c6dc9191ec4fc989e0d` passed 58 tests, strict local lint, complete type-check, changed-file formatting, and independent review. GitHub run `34541597111` passed offline validation (including native serialization) and the connected production build. Vercel Preview `6wzgjMFwE1A4HLABpDgzov3MPcAN` was used for the browser acceptance below. Subsequent release/deployment results are recorded in the PR.

| Check | Observed result |
| --- | --- |
| Native query and pagination | The read-only `verify-resource-state-search.mjs` script passed every six-item page with stable native totals and no duplicate IDs. All states and Maya returned 12 resources; Daniel returned 11; Florida plus nationwide returned 10; nationwide-only and an empty license list returned 9. |
| Daniel workspace | `daniel.04` showed Illinois and Texas workers compensation recommendations with correct state labels. His profile remained licensed in Illinois and Texas. |
| Daniel Search default | My licensed states was selected, with Illinois, Texas + nationwide shown. Page one contained six resources and page two five. No Florida-only article appeared. |
| Broader research and filtering | All states restored 12 results and page one. Workers compensation returned five with All states and four after Clear filters restored the licensed default while keeping the query. Adding State guidance returned Illinois and Texas only; explicitly selecting Florida returned the Florida guide. |
| Saved educational article | Daniel saved the explicitly selected Florida article. After clearing filters, it remained available in Saved resources and its article opened. The test bookmark was removed and the saved count returned to its original zero. |
| Empty-state recovery | An unmatched query under All states returned zero. Reset search cleared the query, restored My licensed states and returned 11 resources. |
| Contrasting agent | After sign-out and fresh `maya.04` login, My licensed states returned 12, with Texas, Florida and Illinois shown. State guidance returned all three state articles. Desktop Search layout was visually inspected. |

The eight new unit tests also cover missing licenses, nationwide mapping, specialization relevance and tied recommendations across licenses. These results demonstrate custom query composition using native SitecoreAI Search and the authenticated synthetic agent adapter. They do not establish a new native UDL decision rule, regulatory approval, or a transaction authorization change. No agent license or CMS resource was modified.
