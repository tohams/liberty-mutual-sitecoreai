...
set -euo pipefail
if [[ $# -lt 1 || "$1" == "--help" ]]; then
  echo 'Usage: authoring/scripts/deploy-content.sh ENVIRONMENT [--seed | --seed-taxonomy] [--publish] [--what-if]'
  echo 'Normal release updates Model and SitePresentation only. --seed creates missing editorial and taxonomy items; --seed-taxonomy creates only missing metadata lists and options.'
  exit 0
fi
...
for portal_option in "$@"; do
  case "$portal_option" in
    --seed) portal_seed=true; portal_seed_taxonomy=true ;;
...
for include in d['items']['includes']:
    if (include['path']!='/sitecore/content/LibertyMutual' or include['allowedPushOperations']!='CreateOnly'
        ...
        raise SystemExit('Refusing seed push: content must remain CreateOnly with only site placeholder and taxonomy exclusions.')
...
if len(deployed)!=3 or set(deployed)!={'nextjs-starter','LibertyMutual.Model','LibertyMutual.SitePresentation'}:
    raise SystemExit('Refusing deployment: editorial content and taxonomy must remain outside authoring resource packages.')
PY
