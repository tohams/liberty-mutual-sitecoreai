...
from component_library_model import apply_rendering_metadata, MANIFEST as COMPONENT_LIBRARY
...
def save(item,kind):
 if kind=='renderings' and item['Path'].rsplit('/',1)[-1] in COMPONENT_LIBRARY['renderings']:
  apply_rendering_metadata(item)
...
# Library grouping and membership are generated by build-component-library.py.
folders={}
...
for n,(slug,title,headline,body,label,url) in enumerate(top):
 ...
 pageids[slug]=page(slug,title,components,order=(n+2)*100)
