...
Production browser checks selected the principal, producer and account-manager copies and left the unrelated specialist on neutral content. Principal and account-manager initially required a refresh. The application now supplies a verified UDL identifier from the signed portal session to the SDK's public personalization API, through a request-scoped subclass of the native PersonalizeProxy. Native discovery, rule execution, variant validation and rewrites remain in the SDK. Browser identity events support measurement rather than deciding the first request's identity. Fresh-login verification of this correction remains pending deployment.
